...
The `.simplePhpCache` subdirectory (or the configured `$cacheDirectoryName`) is
created with mode `0770` when it does not already exist. The implicit fallback base directory is the system temporary
...
- Ensure the base directory and its cache subdirectory are owned and writable
  only by the web server user, and do not allow untrusted users to create
  entries there.
